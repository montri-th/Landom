...
| `public/` | `dist/public/` | Approved, repository-owned assets, including immutable motif runtime/canonical SVG sources and pinned Landom artifact-local quiet-gray fallback derivatives |
...
The page keeps the owner-approved static horizontal Landometer lockup and the established favicon/social-image role boundaries. Motif release `1.2.1`, family `landometer.motif.v3`, supplies separately governed animated decoration; its runtime and canonical quiet SVG source bytes remain unchanged and provenance-pinned. On 2026-09-10 the owner approved a Landom-page-only quiet-gray presentation override and color-only derived fallbacks under `owner_instruction_2026-09-10_quiet_motif_contrast_gray`. This is not a canonical motif 1.2.1 recolor, does not alter identity roles, and is not a portable Design System rule. Photo parallax remains an explicit owner-directed Landom behavior even though Design System 0.9.1 prohibits it. Because v0.9.1 has no artifact exception channel, the released page must not claim zero-exception v0.9.1 conformance while parallax remains enabled. This declaration does not relax any other applicable resolved rule.
...
| `<lm-motif>` and governed child fallback `<img>` | Motif 1.2.1 progressive-enhancement surface with Landom-only quiet-gray presentation; complete derived final state exists before runtime upgrade |
...
Motif release `1.2.1` uses exact local runtime bytes at `public/assets/landometer/landometer-motifs.css` (SHA-256 `7cc2deb475a8d6e4af331407b2b4b741716c458a8ce885e2fb2859374b93912e`) and `public/assets/landometer/landometer-motifs.js` (SHA-256 `3a5caef7918a85885b61dd53e049ea8bf2b0a3cea508f587bb14970bfe6deaf2`). All six role-scoped animated surfaces select the quiet runtime record. The six canonical Energy Sky quiet SVG sources and their release hashes remain unchanged in `docs/assets-manifest.json`; active fallback delivery uses these owner-approved Landom derivatives:

| Canonical quiet source | Page role / theme | Active Landom derived fallback | Ink | Bytes | SHA-256 |
|---|---|---|---|---:|---|
...
| `landometer.dial.quiet` | Directory opener / light foundation | `public/assets/landometer/svg/dial-quiet-gray.svg` | `#5F635A` | 733 | `d24f7a4bcf77a9630720a6c7610ede61a1b23503f13f6db5e87bb3557c5c5b22` |
| `landometer.dial.quiet` | Directory opener / dark foundation | `public/assets/landometer/svg/dial-quiet-gray-dark.svg` | `#C4CECA` | 733 | `637aa8bc06943249658a094904921c4cb10151f775c44fcdc6a0134d71ac0529` |
| `landometer.layers.quiet` | Profile/detail hover / light foundation | `public/assets/landometer/svg/layers-quiet-gray.svg` | `#5F635A` | 1,274 | `c46071e7d7c3d183f85a011b169ca7df0b47777f164852f014171c75b4e1fcc7` |
| `landometer.layers.quiet` | Profile/detail hover / dark foundation | `public/assets/landometer/svg/layers-quiet-gray-dark.svg` | `#C4CECA` | 1,274 | `3d6870c036e25a263296da0fd177a933abc8809581086f7f6757b3d524e2cd99` |
...
Landom host CSS changes only runtime `currentColor`. Luminous Hero/footer surfaces remain on the light-side gray `#5F635A` in both themes; directory/profile foundation surfaces use `#5F635A` in light theme and `#C4CECA` in dark theme. Every derived fallback changes only the canonical source color `#59D2FE` to the applicable gray. Geometry, authored opacity, motion, replay cadence, sizes, and positions stay unchanged. The Hero rings therefore retain their deliberately subtle host opacity, and this record does not claim that every translucent decorative segment reaches 3:1 contrast.

Static or generated source markup renders each matching complete derived fallback before runtime enhancement. The runtime manifest remains `finite_once`; the owner's approval for artifact-local loop/replay is a Landom page-controller decision, not a portable default or a mutation of the release enum, and its cadence remains unchanged. The quiet logo reaches its authored final state at 3,360 ms; hold the complete final state from at least 3,400 ms before replay. A surface runs only while at least 14% visible, the document is visible, reduced motion is not requested, and page motion is not paused. One controller owns timers and invalidates stale asynchronous work on offscreen exit, pause, `visibilitychange`, `pagehide`, and teardown; persisted `pageshow` resumes only eligible surfaces. Reduced motion, print, no JavaScript, dependency failure, and stopped states retain the recorded quiet-gray derived SVG final state. Automatic replay does not use an ARIA live region. The animated quiet logo remains a Hero-only opening treatment; the official navigation logo stays static and authoritative.
