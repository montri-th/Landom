...
| `src/approach-motion.js` | `dist/src/approach-motion.js` | Fail-open, once-only Design System 0.9.1 Riddim approach-motion adapter |
| `src/media-parallax.js` | `dist/src/media-parallax.js` | Bounded owner-directed photo-parallax divergence for explicitly marked photographs |
| `src/brand-motion.js` | `dist/src/brand-motion.js` | Page lifecycle, eligibility, replay, and pause controller for exact motif 1.2.1 components |
| `src/styles.css` | `dist/src/styles.css` | Shared visual, responsive, navigation, and motion states |
| `public/` | `dist/public/` | Approved, repository-owned assets, including immutable motif runtime/canonical SVG sources and exactly four pinned Landom artifact-local quiet-gray fallback derivatives for low-contrast luminous surfaces |
...
### Design System and owner-directed boundary

The active authority for this Landom release is Landometer Design System `0.9.1`, authoring revision `0.9.1-r8`, ruleset `lds-rules-0.9.1`, machine package `v0.9.1-mp7`. Audience colors resolve from exact `color-srgb-05.production.css` bytes, SHA-256 `3bac2499df594bbf6b016b650ee7763f7ec093e33bc5f28239144e0677281d5c`; raw token, scale, and delivery provenance CSS files are not public styling inputs. The earlier unified-navbar handoff, Riddim proposal, and `rebuild02` comparison remain historical implementation evidence only.

The page keeps the owner-approved static horizontal Landometer lockup and the established favicon/social-image role boundaries. Motif release `1.2.1`, family `landometer.motif.v3`, supplies separately governed animated decoration; its runtime and canonical quiet SVG source bytes remain unchanged and provenance-pinned. On 2026-09-10 the owner superseded the broad Landom-page gray treatment with the selective instruction `owner_instruction_2026-09-10_quiet_motif_contrast_gray_selective`: gray is active only for logo, rings, slice, and cultivate on low-contrast luminous Hero/footer surfaces, while dial and layers retain canonical Energy Sky on foundation/card surfaces in both themes. This is not a canonical motif 1.2.1 recolor, does not alter identity roles, and is not a portable Design System rule. Photo parallax remains an explicit owner-directed Landom behavior even though Design System 0.9.1 prohibits it. Because v0.9.1 has no artifact exception channel, the released page must not claim zero-exception v0.9.1 conformance while parallax remains enabled. This declaration does not relax any other applicable resolved rule.
...
| `[data-approach]`, `[data-approach-sequence]` | Explicit Design System 0.9.1 semantic approach-motion units; unmarked content remains in its final state |
| `img[data-parallax-media]` | Owner-directed photo-parallax divergence allowlist; unmarked media remain static |
| `<lm-motif>` and governed child fallback `<img>` | Motif 1.2.1 progressive-enhancement surface with a role-matched final state before runtime upgrade: four Landom-only quiet-gray derivatives on low-contrast luminous surfaces and canonical Energy Sky dial/layers on foundation/card surfaces |
...
The calm header is a progressive visual state, not a content or hit-target reduction contract. Every discrete action retains its own direct semantic target of at least 44 × 44 CSS px in every state; do not scale controls into a smaller visual target and forward activation through a transparent proxy. Captured scroll events may keep the last position per scroller and return the header to prominence at the top, on upward scroll, during pointer/keyboard intent, while the menu is open, and whenever `prefers-reduced-motion: reduce` matches. The CTA discovery cue is the governed first-entry treatment: once per load, 540 ms maximum, `cubic-bezier(.16,1,.3,1)`, a 28% yellow band moving from -120% to +120%, no flicker, no perpetual loop, and a fully readable static equivalent for reduced motion or observer failure.

`src/approach-motion.js` implements `motion-riddim-approach-02` only for explicit eligible roles: `approach.soft`, `approach.inline-start`, `approach.inline-end`, `media.arrival`, and `stagger.child`. Opacity resolves in 760 ms, transform in 920 ms, media arrival in 900 ms, block and logical-inline distances are 32 and 36 px, scale starts at `.985`, and stagger advances 150 ms per beat capped at 450 ms. The adapter can register eligible units inserted after initial load, but settled keys are never re-armed. One shared observer uses threshold `0.14`, bottom root margin `-12%`, and a 2,400 ms initialization watchdog. Unsupported APIs, runtime errors, print, reduced motion, BFCache restore, deep-link/focus entry, opened disclosures, and hidden or critical content fail open to the fully visible and operable final state. Hero/LCP content, navigation, live regions, page-owning headings, controls, and critical proof are excluded from decorative entrance motion.

Direct `?person=` routes land their target card before expansion and scroll. User-driven profile expansion or collapse lands the directory sequence before its FLIP reflow begins, so reveal and reflow never compete for the same card transform.

`src/media-parallax.js` is a separate, component-local owner-approved enhancement and an explicit divergence from the Design System 0.9.1 prohibition on parallax. Only explicitly marked photographs participate: the governed Hero anchor, the three governed Hero moments, and governed person portraits. Logos, navigation/social icons, animated motifs and logo, certificate thumbnails, certificate dialogs, and other evidence/document reading surfaces remain unmarked and static. `IntersectionObserver` limits work to visible images; a passive scroll listener schedules a single `requestAnimationFrame` update with requested depth capped at 36 px. Runtime translation is further capped by the actual bleed created by each image's scale inside its clipping frame, with a one-pixel safety inset so no frame background is exposed. Unsupported APIs, `prefers-reduced-motion: reduce`, print, `Save-Data`, page hiding, and teardown restore the original static transform without delaying image content. Owner approval records the requested behavior but cannot be represented as a Design System exception or conforming implementation.

Motif release `1.2.1` uses exact local runtime bytes at `public/assets/landometer/landometer-motifs.css` (SHA-256 `7cc2deb475a8d6e4af331407b2b4b741716c458a8ce885e2fb2859374b93912e`) and `public/assets/landometer/landometer-motifs.js` (SHA-256 `3a5caef7918a85885b61dd53e049ea8bf2b0a3cea508f587bb14970bfe6deaf2`). All six role-scoped animated surfaces select the quiet runtime record. The six canonical Energy Sky quiet SVG sources and their release hashes remain unchanged in `docs/assets-manifest.json`; active fallback delivery uses four owner-approved Landom gray derivatives on low-contrast luminous Hero/footer surfaces and the exact canonical quiet source for dial/layers on foundation/card surfaces:

| Canonical quiet source | Page role / surface | Active fallback | Ink | Bytes | SHA-256 |
|---|---|---|---|---:|---|
| `landometer.logo.quiet` | Hero brand opening / luminous | `public/assets/landometer/svg/logo-quiet-gray.svg` | `#5F635A` | 1,979 | `6be5a0a6095a85d601e02402fe1466a0c07247ec25fe12e73a2229823ef6e4c3` |
| `landometer.rings.quiet` | Hero spatial depth / luminous | `public/assets/landometer/svg/rings-quiet-gray.svg` | `#5F635A` | 833 | `9d171e8a5fbe9c149e1ee9c5e3bfac60486effe80b450490bacc0dcca33bd3da` |
| `landometer.dial.quiet` | Directory opener / foundation, both themes | `public/assets/landometer/svg/dial-quiet.svg` | `#59D2FE` | 733 | `2e624d80b604891ad2ed3e4d5cc6268d2383f39f740ef1def5012c49aee0da1f` |
| `landometer.layers.quiet` | Profile/detail hover / card, both themes | `public/assets/landometer/svg/layers-quiet.svg` | `#59D2FE` | 1,274 | `e3e2bf65bcd38d34d0a07910bef44917eab76fdaf097131ec133d163f6a65a03` |
| `landometer.slice.quiet` | Action/map hover / luminous | `public/assets/landometer/svg/slice-quiet-gray.svg` | `#5F635A` | 350 | `73db5c884cdcfd2014d5709f3aa5833c3db775f8c858804c66aa3ede826bf9f3` |
| `landometer.cultivate.quiet` | Footer closer / luminous | `public/assets/landometer/svg/cultivate-quiet-gray.svg` | `#5F635A` | 1,190 | `8ba9ca8abc66c7a11694526a682b980be0c7856decff77b8d3a99552bde9402f` |

Landom host CSS changes only runtime `currentColor` for logo, rings, slice, and cultivate, which use `#5F635A` on luminous Hero/footer surfaces in both themes. Dial and layers inherit canonical Energy Sky `#59D2FE` on foundation/card surfaces in both themes. Each of the four active gray fallbacks changes only the canonical source color `#59D2FE` to `#5F635A`. Geometry, authored opacity, motion, replay cadence, sizes, and positions stay unchanged. The Hero rings therefore retain their deliberately subtle host opacity, and this record does not claim that every translucent decorative segment reaches 3:1 contrast.

Static or generated source markup renders each role-matched complete fallback before runtime enhancement: the approved gray derivative for logo/rings/slice/cultivate and the canonical Energy Sky quiet source for dial/layers. The runtime manifest remains `finite_once`; the owner's approval for artifact-local loop/replay is a Landom page-controller decision, not a portable default or a mutation of the release enum, and its cadence remains unchanged. The quiet logo reaches its authored final state at 3,360 ms; hold the complete final state from at least 3,400 ms before replay. A surface runs only while at least 14% visible, the document is visible, reduced motion is not requested, and page motion is not paused. One controller owns timers and invalidates stale asynchronous work on offscreen exit, pause, `visibilitychange`, `pagehide`, and teardown; persisted `pageshow` resumes only eligible surfaces. Reduced motion, print, no JavaScript, dependency failure, and stopped states retain that role-matched final state. Automatic replay does not use an ARIA live region. The animated quiet logo remains a Hero-only opening treatment; the official navigation logo stays static and authoritative.

Navigation symbols are served locally from the canonical Design System 0.9.1 `public/assets/fonts/material-symbols-rounded-nav-300.woff2` bytes (2,500 bytes, SHA-256 `d7e283106ed2898726b24504c4e0f5ad524292984a90a4d29553c7dcf53b9657`) at `FILL 0, wght 300, GRAD 0, opsz 24`, containing `open_in_new`, `menu`, `close`, `light_mode`, `dark_mode`, `contrast`, and `groups`. The footer map capsule uses the exact governed artifact-local `rebuild02-footer-r10` Material Symbols bytes at `public/assets/fonts/material-symbols-rounded-footer-r10.ttf` (9,464 bytes, SHA-256 `bbcc034717d243cd5a3653dd1169cec00e8f11f289c20ef949078db24dc680f5`) for its `map` glyph. The filled `groups` font formerly used only by the fixed rail is not referenced, preloaded, or listed in the active font manifest. The earlier canonical `open_in_new` subset remains separately governed because contribution links use it. Keep the shared Apache-2.0 license and the footer subset's local authority boundary explicit in `public/assets/fonts/font-assets.manifest.json`.

The footer is a local adaptation of the contact/footer visual family observed on `rebuild02/Landometer-Home-TH.dc.html`; it is not a source dependency or a normative Design System release. It deliberately omits the reference Hello form. The office-map action matches the reference's 44 px icon-plus-label capsule, outline Material `map` glyph, and separate external-link cue rather than the former underlined Unicode marker. The current footer retains the approved Landometer horizontal identity, company/address/map/email details, real `#top` and `#people` links, the approved privacy destination, and five verified corporate social profiles: Facebook (`https://www.facebook.com/landometer`), Instagram (`https://www.instagram.com/landometer`), TikTok (`https://www.tiktok.com/@landometer82`), LinkedIn (`https://www.linkedin.com/company/landometer`), and X (`https://x.com/landometer`).
...
Browser tabs under the current Design System 0.9.1 artifact binding retain the exact transparent compact symbol at `https://montri-th.github.io/Landometer/assets/images/landometer-symbol-transparent.png?v=35a1496f` (PNG, 192×192, 11,001 bytes, SHA-256 `35a1496f6e8c502cef82f0a46de5dacff98718ff9f5a6c07ccc3783d76e3ae85`). The original exact-byte approval lineage remains recorded in `docs/identity-discovery.json`; the role is favicon-only. The horizontal header lockup remains prohibited as a favicon, compact icon, person avatar, or `og:image`; the compact symbol is likewise not approved for apple-touch, maskable/install, social preview, or horizontal header roles.
...
The current Hero uses the owner-selected community-constellation composition. Its anchor is `public/assets/hero/landom-community-anchor.jpg`; its circular moments use `landom-community-dinner.jpg`, `landom-community-gathering.jpg`, and `landom-community-citymeter.jpg`. The CityMETER moment replaces only the prior work-session circle and preserves the existing layout. Every photograph is an owner-supplied, deterministic crop/resize with metadata removed; exact dimensions, hashes, source filenames, and role restrictions are pinned in `docs/assets-manifest.json`. The earlier `public/assets/decor/landometer-hero-motif.png` remains a governed historical decorative derivative but is not an animated identity asset. Current brand motion uses only the exact motif 1.2.1 runtime, the canonical quiet SVGs for all six source records, and the four role-matched owner-approved Landom quiet-gray derivatives for low-contrast luminous surfaces; none replaces the compact favicon, official horizontal lockup, social preview, or person identity.
...
2. `deploy`: GitHub Pages deployment followed by a release-SHA cache-busted fetch. The live `build-manifest.json` must first match the exact digest emitted by this workflow's build job; only then may it attest manifest-hash parity for HTML, public JSON, application/navigation/approach/parallax/brand-motion JavaScript, audience CSS, discovery files, active governed Material Symbols assets, the official lockup, all six canonical quiet SVG sources, exactly four active Landom quiet-gray derived fallbacks, both exact motif runtime files, and governed social-preview/Hero/portrait/certificate assets. The live smoke test also checks status/MIME, localized menu routes, direct control geometry, finite CTA cue, required unified-navigation markers, absence of the retired fixed rail and dead certificate bookmarks, motion fail-open/lifecycle guards, the explicitly divergent photo-parallax fail-static boundary, corporate-footer destinations, exact source/build/live hashes, and color-only normalization of each derivative back to its canonical source.
...
- prominent/calm header transitions in both scroll directions, including direct 44 × 44 targets, no proxy activation, pointer/focus intent, and menu-open override;
...
- Riddim approach motion at normal motion with exact 0.9.1 roles/timings, plus no hidden or delayed content under reduced motion, print, deep-link/focus entry, and BFCache restore;
- finite first-entry CTA sweep at normal motion and complete static CTA under reduced motion or observer failure, with no flicker or repeat;
- all six quiet motif surfaces, with gray derivatives for logo/rings/slice/cultivate and canonical Energy Sky fallbacks for dial/layers at 320, 360, 390, tablet, and wide desktop widths, both themes, 130% and 200% text scale, no JavaScript, print, runtime failure, pause/resume, offscreen exit, visibility changes, and BFCache; inspect at least two replay cycles, verify no timer/DOM/request accumulation, and treat the unchanged low-opacity Hero rings as intentionally subtle rather than claiming universal 3:1 contrast;
- owner-directed photo parallax at normal motion, plus static Hero/portrait media under reduced motion, print, save-data, unsupported APIs, and page lifecycle resets; verify that logos, icons, motifs, and certificate reading surfaces never move and do not claim zero-exception 0.9.1 conformance;
