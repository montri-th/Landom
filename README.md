...
- Approved portrait metadata lives in `data/approved/portrait-assets.json`; public hashes and role restrictions are also recorded in `docs/assets-manifest.json`. The horizontal Landometer logo remains approved only as the header lockup and is not a favicon or person avatar. Under the current Design System v0.9.1 artifact binding, browser tabs retain the exact 192×192 transparent compact symbol for favicon duty only; its URL, hash, dimensions, alpha, original approval lineage, and boundaries are pinned in `docs/identity-discovery.json`.
- This release replaces only the blank white, context-free backgrounds for `I0001`, `I0008`, `I0012`, `I0018`, `I0019`, `I0021`, `I0025`, `I0033`, and `I0035`. It composites approved DS gradients through a pixel-preserving foreground mask, keeps environmental portraits unchanged, records the exact approved recipe per asset, uses multiple recipes across the set, and assigns Diversity Spectrum to exactly one portrait. Edge-refined v2 then adjusts only the foreground-mask boundary to reduce residual light fringe while preserving the crop, portrait interior, gradient recipe, identity, rights, and publication basis.
...
- `/Landom/` is both the Thai canonical route and `x-default`; `/Landom/en/` is a crawlable English build. The two routes use reciprocal `hreflang`, localized initial HTML, localized Open Graph/Twitter records with an owner-approved collaboration photograph, truthful `CollectionPage` JSON-LD, and matching sitemap entries without a duplicate Thai path. `llms.txt` is a public-safe navigation aid to the canonical pages and generated JSON; it is not permission, a ranking signal, a license, evidence, or agent authority. No apple-touch or maskable/install icon is claimed because those roles do not yet have separate approval. Search-result favicon selection, hostname-root robots behavior, and third-party social-preview cache refresh remain outside an individual project subpath's control.

## Interface contract

The current visual and interaction authority is **Landometer Design System 0.9.1**, authoring revision **0.9.1-r8**, ruleset **`lds-rules-0.9.1`**, machine package **`v0.9.1-mp7`**. Audience color roles resolve from the exact `color-srgb-05.production.css` projection, SHA-256 `3bac2499df594bbf6b016b650ee7763f7ec093e33bc5f28239144e0677281d5c`; provenance token/scales/delivery CSS is not shipped as audience styling. Historical navbar and Riddim proposals remain implementation lineage only and do not override the active release.

Desktop navigation exposes CityMETER, CityWiki, the join-team CTA, and a menu; mobile keeps the product identity and menu trigger. The menu owns display preferences, a real sibling-language route, ecosystem destinations, and the only truthful page bookmark (`#people`). The previous fixed desktop bookmark rail stays removed because it duplicated that single shortcut. Every state keeps direct semantic targets of at least 44 × 44 CSS px without activation forwarding through a scaled proxy. Focus containment, Escape close, focus return, responsive visibility, and calm/prominent header states are enforced by validation. The CTA discovery sweep is finite and once-per-load; reduced motion and observer failure expose the complete, operable static label immediately. Navigation symbols stay outline-only at `FILL 0`, weight 300; the retired filled `groups` font is not part of the active interface.

Decorative Riddim approach motion uses the approved `motion-riddim-approach-02` roles and is progressive and fail-open. It applies only to explicitly marked, noncritical semantic units, runs once, and resolves immediately to final visible content for reduced motion, print, focus/deep-link entry, BFCache restore, unsupported APIs, or runtime errors. Hero/LCP content, navigation, controls, and live regions are excluded.

Animated brand surfaces select the quiet records from motif release **1.2.1**, family **`landometer.motif.v3`**, for all six kinds: `logo`, `rings`, `dial`, `layers`, `slice`, and `cultivate`. The canonical runtime and six Energy Sky quiet source SVGs remain byte-for-byte unchanged and provenance-pinned. Under the owner's superseding 2026-09-10 selective instruction (`owner_instruction_2026-09-10_quiet_motif_contrast_gray_selective`), site CSS changes runtime `currentColor` only for `logo`, `rings`, `slice`, and `cultivate` on low-contrast luminous Hero/footer surfaces; no-JavaScript and stopped states use their four matching color-only gray derivatives in `#5F635A`. `dial` and `layers` retain canonical Energy Sky `#59D2FE` and their exact canonical quiet fallbacks on foundation/card surfaces in both themes. This is a Landom artifact-local contrast treatment, not a canonical motif 1.2.1 recolor or portable Design System rule. Geometry, authored opacity, motion, replay, sizes, and positions are unchanged; the Hero rings remain intentionally subtle at their existing host opacity, so no universal 3:1 claim is made for translucent decorative segments.

The quiet logo reaches its authored final state at 3,360 ms and remains held from at least 3,400 ms before replay. It is a Hero brand-opening treatment and never replaces the official static navbar/footer lockup, favicon, or social identity. The owner-approved artifact-local loop/replay cadence remains unchanged and is not a portable Design System default. One controller owns visibility, replay, pause/resume, reduced-motion, print, page lifecycle, and failure behavior.

Photo parallax remains an owner-directed Landom site behavior for the governed Hero photographs and person portraits. It is a deliberate divergence from the Design System v0.9.1 prohibition on parallax. It is bounded, subordinate, and fail-static for reduced motion, print, save-data, unsupported APIs, and lifecycle teardown, but this owner decision does not create an exception channel in the Design System. Consequently this page applies the other resolved v0.9.1 rules but does **not** claim zero-exception v0.9.1 artifact conformance while parallax remains enabled.

The footer adapts the visual structure of the current `rebuild02` comparison surface but deliberately omits its Hello form. It keeps the approved Landometer identity, contact details, real page links, and the five verified corporate profiles: Facebook, Instagram, TikTok, LinkedIn, and X. This corporate-footer allowlist is separate from person records: public individual profile controls remain LinkedIn and GitHub only.
