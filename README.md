...
Desktop navigation exposes CityMETER, CityWiki, the join-team CTA, and a menu; mobile keeps the product identity and menu trigger. The menu owns display preferences, a real sibling-language route, ecosystem destinations, and the only truthful page bookmark (`#people`). Its focus containment, Escape close, focus return, responsive visibility, and calm/prominent header states are enforced by validation. The owner-approved local r7 interaction uses the exact 76→29 px desktop and 68→27 px mobile deep-calm treatment, captured per-scroller direction, the bounded `lmSweep`/`lmFlick` CTA text highlight on desktop and mobile, and immediate final states under reduced motion.
