...
Animated brand surfaces use motif release **1.2.1**, family **`landometer.motif.v3`**, with exact local runtime bytes and exact SVG final-state fallbacks. The animated full logo is a Hero brand-opening treatment; it never replaces the official static navbar/footer lockup, favicon, or social identity. The owner approved artifact-local loop/replay for this page; it is not a portable Design System default. One controller owns visibility, replay, pause/resume, reduced-motion, print, page lifecycle, and failure behavior; no-JavaScript and stopped states retain the exact final SVG.
