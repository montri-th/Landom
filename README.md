...
Animated brand surfaces use motif release **1.2.1**, family **`landometer.motif.v3`**, with the exact quiet record and exact SVG final-state fallback for all six kinds: `logo`, `rings`, `dial`, `layers`, `slice`, and `cultivate`. The quiet logo reaches its authored final state at 3,360 ms and remains held from at least 3,400 ms before replay. It is a Hero brand-opening treatment and never replaces the official static navbar/footer lockup, favicon, or social identity. The owner-approved artifact-local loop/replay cadence remains unchanged and is not a portable Design System default. One controller owns visibility, replay, pause/resume, reduced-motion, print, page lifecycle, and failure behavior; no-JavaScript and stopped states retain the exact quiet final SVG.
