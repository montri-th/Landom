...
Desktop navigation exposes CityMETER, CityWiki, the join-team CTA, and a menu; mobile keeps the product identity and menu trigger. The menu owns display preferences, a real sibling-language route, ecosystem destinations, and the only truthful page bookmark (`#people`). The previous fixed desktop bookmark rail has been removed because it duplicated that single shortcut. Its focus containment, Escape close, focus return, responsive visibility, and calm/prominent header states are enforced by validation. The owner-approved local r7 interaction uses the exact 76→29 px desktop and 68→27 px mobile deep-calm treatment, captured per-scroller direction, the bounded `lmSweep`/`lmFlick` CTA text highlight on desktop and mobile, and immediate final states under reduced motion. The outline navigation-symbol subset remains local; the filled `groups` font formerly used only by the active rail is no longer used or preloaded.
...
Photo parallax is a separate, component-local enhancement rather than a normative Design System rule. Explicit markers opt in only the four governed Hero photographs and governed person portraits; logos, icons, the Hero motif, and certificate/evidence images remain static. `IntersectionObserver` limits work to visible media, passive scroll events schedule one `requestAnimationFrame` update, and reduced-motion, print, save-data, or unsupported-browser paths fail static without delaying content.
