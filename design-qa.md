...
- Animated-brand authority: motif release `1.2.1`, family `landometer.motif.v3`, using exact role-scoped runtime and canonical source bytes recorded in `docs/assets-manifest.json`. Active fallbacks are separately pinned Landom artifact-local quiet-gray derivatives.
...
- On 2026-09-10 the owner approved a Landom-page-only quiet-gray contrast override (`owner_instruction_2026-09-10_quiet_motif_contrast_gray`). It changes only runtime `currentColor` and the matching fallback color; it is not a canonical motif 1.2.1 recolor or a portable Design System rule.
...
The exact runtime remains `public/assets/landometer/landometer-motifs.css` plus `public/assets/landometer/landometer-motifs.js`, and the six canonical Energy Sky quiet SVG sources remain byte-for-byte unchanged and provenance-pinned in the manifest. Landom host CSS changes only `currentColor`: luminous Hero/footer surfaces use `#5F635A`; directory/profile foundation surfaces use `#5F635A` in light theme and `#C4CECA` in dark theme. Each active fallback is derived from its canonical source by replacing only `#59D2FE` with the applicable gray. Geometry, authored opacity, motion, replay cadence, size, and position remain unchanged. These files are owner-approved site derivatives, not exact motif-release fallbacks.

Static or generated source markup keeps the matching complete derived fallback visible until `<lm-motif>` upgrades and its dependencies are ready. The manifest motion mode remains `finite_once`; the existing artifact-local loop/replay cadence remains a separately owner-approved Landom page-controller decision and is not a portable Design System default. Quiet-logo animation reaches its authored final state at 3,360 ms; hold the complete final state from at least 3,400 ms before any replay. Do not use the historical 2,050 ms cutoff or a cyan wedge override. The Hero rings retain their intentionally subtle host opacity; this implementation does not claim that every translucent decorative segment reaches a 3:1 contrast ratio.

One controller owns each timer and the page-level pause/resume state. A surface is eligible only when at least 14% visible, the document is visible, reduced motion is not requested, and page motion is not paused. Offscreen, `visibilitychange`, `pagehide`, reduced motion, print, runtime failure, and no JavaScript retain the matching complete quiet-gray derived SVG state. Persisted `pageshow` resumes only eligible surfaces. Automatic cycles do not announce through an ARIA live region.
...
- exact MIME type, SHA-256, source-to-build parity, and live bytes for both immutable motif runtime files, all six canonical quiet source SVGs, and all eight active quiet-gray derived fallback SVGs; color-only derivation must normalize byte-for-byte back to its recorded canonical source;
...
| P0 | Reduced motion, print, no JavaScript, and failed motif runtime show complete content, actions, and the recorded quiet-gray derived SVG final states. | Fresh fail-open screenshots/runtime assertions |
...
| P0 | Motif CSS/JavaScript, six canonical sources, and eight active derived fallbacks match the recorded hashes in source, build, and canonical HTTPS responses; each derivative differs only by its approved color substitution. | Build-manifest and deployed-byte receipt |
