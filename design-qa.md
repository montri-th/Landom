...
- Animated-brand authority: motif release `1.2.1`, family `landometer.motif.v3`, using exact role-scoped runtime and canonical source bytes recorded in `docs/assets-manifest.json`. Active fallbacks are the canonical Energy Sky sources for dial/layers and separately pinned Landom artifact-local quiet-gray derivatives for logo/rings/slice/cultivate.
...
- On 2026-09-10 the owner superseded the broad Landom-page gray treatment with a selective contrast instruction (`owner_instruction_2026-09-10_quiet_motif_contrast_gray_selective`): only logo, rings, slice, and cultivate use gray where Energy Sky merges with luminous Hero/footer surfaces; dial and layers retain canonical Energy Sky on foundation/card surfaces in both themes. It is not a canonical motif 1.2.1 recolor or a portable Design System rule.
...
| `landometer.slice.quiet` | Action/map CTA hover / luminous | `public/assets/landometer/svg/slice-quiet-gray.svg` | `#5F635A` | 350 | `73db5c884cdcfd2014d5709f3aa5833c3db775f8c858804c66aa3ede826bf9f3` |
...
The exact runtime remains `public/assets/landometer/landometer-motifs.css` plus `public/assets/landometer/landometer-motifs.js`, and the six canonical Energy Sky quiet SVG sources remain byte-for-byte unchanged and provenance-pinned in the manifest. Landom host CSS changes only `currentColor` for the four low-contrast luminous-surface motifs: logo, rings, slice, and cultivate use `#5F635A` in both themes. Dial and layers inherit canonical Energy Sky `#59D2FE` and use their exact canonical quiet SVGs as fallbacks on foundation/card surfaces in both themes. Each of the four active gray fallbacks is derived from its canonical source by replacing only `#59D2FE` with `#5F635A`. Geometry, authored opacity, motion, replay cadence, size, and position remain unchanged. These four files are owner-approved site derivatives, not exact motif-release fallbacks.

Static or generated source markup keeps the matching complete fallback visible until `<lm-motif>` upgrades and its dependencies are ready: the approved gray derivative for logo/rings/slice/cultivate and the canonical Energy Sky quiet source for dial/layers. The manifest motion mode remains `finite_once`; the existing artifact-local loop/replay cadence remains a separately owner-approved Landom page-controller decision and is not a portable Design System default. Quiet-logo animation reaches its authored final state at 3,360 ms; hold the complete final state from at least 3,400 ms before any replay. Do not use the historical 2,050 ms cutoff or a cyan wedge override. The Hero rings retain their intentionally subtle host opacity; this implementation does not claim that every translucent decorative segment reaches a 3:1 contrast ratio.

One controller owns each timer and the page-level pause/resume state. A surface is eligible only when at least 14% visible, the document is visible, reduced motion is not requested, and page motion is not paused. Offscreen, `visibilitychange`, `pagehide`, reduced motion, print, runtime failure, and no JavaScript retain the matching complete fallback state: gray for logo/rings/slice/cultivate and canonical Energy Sky for dial/layers. Persisted `pageshow` resumes only eligible surfaces. Automatic cycles do not announce through an ARIA live region.
...
- exact MIME type, SHA-256, source-to-build parity, and live bytes for both immutable motif runtime files, all six canonical quiet source SVGs, and exactly four active quiet-gray derived fallback SVGs; each color-only derivative must normalize byte-for-byte back to its recorded canonical source;
...
| P0 | Reduced motion, print, no JavaScript, and failed motif runtime show complete content, actions, and the role-matched final states: four recorded quiet-gray derivatives plus canonical Energy Sky dial/layers. | Fresh fail-open screenshots/runtime assertions |
...
| P0 | Motif CSS/JavaScript, six canonical sources, and four active derived fallbacks match the recorded hashes in source, build, and canonical HTTPS responses; each derivative differs only by its approved color substitution. | Build-manifest and deployed-byte receipt |
